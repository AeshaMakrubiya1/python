...
class CovidTool:
    ...
    def look(self):
        if self.df.empty: return
        plt.figure(); self.df.set_index("date")["cases"].rolling(7).mean().plot(); plt.ylabel("7D mean"); plt.title("LINE view"); plt.tight_layout(); plt.savefig("covid_06_line.png"); plt.close()
        plt.figure(); self.df.set_index("date")[["cases","recovered"]].plot(kind="area", alpha=0.5); plt.title("AREA view 2"); plt.tight_layout(); plt.savefig("covid_06_area.png"); plt.close()

def show_menu():
    print("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    print("Mini Studio :: Hey there!")
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
